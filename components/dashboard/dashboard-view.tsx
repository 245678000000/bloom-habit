    ...
    <div className="space-y-4 pb-6">
      <Card className="overflow-hidden">
        <CardContent className="relative py-4">
          ...
          <p className="text-sm text-[#6d8075] dark:text-[#a3b5aa]">{`${greeting}，${state.displayName}～今天也要开花哦`}</p>
          <div className="mt-2 flex items-center gap-2 text-[#4d6357] dark:text-[#cadad1]">
            <CalendarDays className="h-4 w-4" />
            <p className="text-sm">{formatToday(new Date())}</p>
          </div>
          <div className="mt-4">
            <DateStrip
      ...
      >
        <Card className="bg-gradient-to-br from-[#fff5ed]/90 via-[#f0f8f3]/90 to-[#efe8fa]/86 dark:from-[#243129] dark:via-[#1f2c25] dark:to-[#2f2637]">
            ...
            <div className="flex items-center gap-3">
              <motion.div
                animate={
                  reduceMotion
                    ? undefined
                    : { rotate: [0, -6, 6, -4, 0], y: [0, -2, 0] }
                }
                transition={{ duration: 2.8, repeat: Infinity, ease: "easeInOut" }}
                className={cn(!reduceMotion && "fire-sway")}
              >
                <Flame className="h-10 w-10 fill-[#f4a96d] text-[#f08f52]" />
              </motion.div>
              <div>
                <p className="text-4xl font-black leading-none text-[#4b5e54] dark:text-[#d5e5db]">{streak}</p>
                  ...
                  >
                    ...
                    <div className="relative">
                      <Button
                        ...
                        className={cn(
                          "rounded-full px-4",
                          done && "bg-gradient-to-br from-[#9dd1b5] to-[#82be9f] text-white dark:from-[#629b7f]"
                        )}
                        onClick={() => {
                          ...
                          toast({
                            message: willComplete ? `已打卡「${habit.name}」` : `已取消「${habit.name}」`,
                            variant: "success",
                            action: {
                              label: "撤销",
